...
func New(ctx context.Context) (Context, context.CancelFunc) {
	newCtx := Context{modules: make(map[ModuleID]Module)}
...
func (ctx Context) LoadModuleByID(id ModuleID) (any, error) {
	v, ok := ctx.modules[id]
	if ok {
		return v, nil
	}
